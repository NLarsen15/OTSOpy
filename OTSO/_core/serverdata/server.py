...
def GetServerData(Date, External, AdaptiveExternalModel=False):
    ...
    Date, Bx, By, Bz, V, Density, Pdyn, Kp, Dst, G1, G2, G3, W1, W2, W3, W4, W5, W6, ByAvg, BzAvg, NIndex, BIndex, SymHCorrected, External, Kp_raw = ExtractServerData(RoundedDate, External, AdaptiveExternalModel)

    # Use Kp_raw instead of Kp for external = 102
    if External == 100:
        Kp = Kp_raw
...
def load_all_csvs(unique_years):
    ...
    yeardf = pd.concat(yeardfs, ignore_index=True)
...
def ExtractServerData(RoundedDate, External, AdaptiveExternalModel=False):
    ...
    if matching_row.empty:
        ...
        Kp_raw = row_data['Kp_raw'] if 'Kp_raw' in row_data else Kp  # Fallback to Kp if Kp_raw not available
        ...
        if External == 9 or External == 11:
             if not is_invalid_value(NIndex) and NIndex > 2:
                 print(f'WARNING: N-INDEX ({NIndex}) BEYOND TYPICAL RANGE (0-2). RESULTS MAY BE UNRELIABLE.')
                 #NIndex = 2
        ...
        Kp_raw = 0 if is_invalid_value(Kp_raw) else Kp_raw
        ...
        return Date, Bx, By, Bz, V, Density, Pdyn, Kp, Dst, G1, G2, G3, W1, W2, W3, W4, W5, W6, ByAvg, BzAvg, NIndex, BIndex, SymHCorrected, External, Kp_raw
...
def ExtractServerFlightData(RoundedDate, External, datadf, AdaptiveExternalModel=False):
    ...
    matching_row = datadf[datadf['Date'] == RoundedDate]
