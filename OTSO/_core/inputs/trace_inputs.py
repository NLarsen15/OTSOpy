...
def TraceInputs(Data: TraceData) -> None:
     ...
     Data.endparams = [Data.minaltitude, Data.maxdistance, Data.maxtime, Data.maxsteps]
     ...
     if ServerData == 1:
          ...
          BxS, ByS, BzS, VS, DensityS, PdynS, KpS, DstS, G1S, G2S, G3S, W1S, W2S, W3S, W4S, W5S, W6S, ByAvgS, BzAvgS, NIndexS, BIndexS, SymHCorrectedS, External = server.GetServerData(EventDate,External)
     ...
     Data.particlearray = [AtomicNum,AntiCheck]
 
     if Data.latstep > 0:
          Data.latstep = -1*Data.latstep
 
     if not ((Data.maxlat-Data.minlat)/abs(Data.latstep)).is_integer():
           print(f"Latitude range selected can not be split into integer number of steps using inputted latstep. \nlat range:{Data.maxlat-Data.minlat} \nlatstep:{abs(Data.latstep)} \nlatrange/latstep:{(Data.maxlat-Data.minlat)/abs(Data.latstep)}\nPlease edit the inputs appropriately.")
           exit()
 
     if not ((Data.maxlong-Data.minlong)/abs(Data.longstep)).is_integer():
           print(f"Longitude range selected can not be split into integer number of steps using inputted longstep. \nlong range:{Data.maxlong-Data.minlong} \nlongstep:{abs(Data.longstep)} \nlongrange/longstep:{(Data.maxlong-Data.minlong)/abs(Data.longstep)}\nPlease edit the inputs appropriately.")
           exit()
 
     Data.latitudelist = np.arange(Data.maxlat,Data.minlat + Data.latstep,Data.latstep)
     Data.longitudelist = np.arange(Data.minlong,Data.maxlong + Data.longstep,Data.longstep)
