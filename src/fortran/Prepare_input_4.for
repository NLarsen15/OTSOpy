...
c
      SUBROUTINE PREPAREINPUT4(OMNIYEAR, DIRECTORY, length)
      USE GEOPACK1
      implicit real*8 (a-h,o-z)
      REAL AAA,TILT,BBB,VXX,VYY,VZZ

      CHARACTER(length + 80) :: SWNAME,NAMEOUT,LISTNAME,PARFILE
     ...
     * IMFFLAG(105408),ISWFLAG(105408)
      ...
      WRITE(SWNAME, '(A, I0, "_IMF_&_SW_gaps_le_3hrs_filled.txt")')
     & TRIM(DIRECTORY)// "/", IYEAR
      WRITE(LISTNAME, '(A, I0, "_Interval_list.txt")')
     & TRIM(DIRECTORY)// "/", IYEAR
      WRITE(NAMEOUT, '(A, I0, "_OMNI_5m_with_TS05_variables.dat")') 
     & TRIM(DIRECTORY) // "/", IYEAR

      PARFILE = TRIM(DIRECTORY) // "/Parameters.par"
      
C
      OPEN (UNIT=1,FILE=PARFILE)
...
C
 ...
 1    READ (1,505,END=2) IYEA,IDA,IHOU,MI,BX,BY,BZ,V,VX,VY,VZ,DE,T,DST5M,
     * IMFLAG,ISFLAG
...
C      PRINT *, '  READING OF OMNI DATA FINISHED'
C
...
C    CALCULATE W1 - W6 INDICES:
 ...
 43         W1=W1*DT1*5.
     ...
     * TEMP(IND),IMFFLAG(IND),ISWFLAG(IND),
     * TILT,Pdyn,W1,W2,W3,W4,W5,W6
 444  FORMAT (2I4,2I3,3F8.2,3F8.1,F7.2,F9.0,2(3X,I2),F8.4,F7.2,
     * 7F7.2)
c
