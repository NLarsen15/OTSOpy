...
C   (2) TAIL15 MODULE 
C
      CALL DEFORM_XZ_YZ_15N (PS,PARAMETERS,XX,YY,ZZ,TBX,TBY,TBZ,
     *  TAIL15_SHIELDED_15N) 
      TX=PARAMETERS(2)*TBX
      TY=PARAMETERS(2)*TBY
      TZ=PARAMETERS(2)*TBZ
...
C
      CALL DEFORM_XZ_YZ_15N(PS,PARAMETERS,XX,YY,ZZ,SBX,SBY,SBZ,
     *                      SRC_SHIELDED_15N) 
      SX=PARAMETERS(3)*SBX
      SY=PARAMETERS(3)*SBY
      SZ=PARAMETERS(3)*SBZ
...
C
      CALL DEFORM_XZ_YZ_15N(PS,PARAMETERS,XX,YY,ZZ,PBX,PBY,PBZ,
     *                      PRC_SHIELDED_15N) 
      PX=PARAMETERS(4)*PBX
      PY=PARAMETERS(4)*PBY
      PZ=PARAMETERS(4)*PBZ
...
c
      SUBROUTINE TAIL15_SHIELDED_15N (X,Y,Z,PARAMETERS,BX,BY,BZ)   
C
...
c&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
C
      SUBROUTINE DEFORM_XZ_YZ_15N (PS,PARAMETERS,X,Y,Z,BX,BY,BZ,
     *   B_EXT_UNTILTED_15N)
C
...
c
      CALL B_EXT_UNTILTED_15N (XAS1,YAS2,ZAS2,PARAMETERS,BX_AS2,BY_AS2,
     *  BZ_AS2)
C
...
C
      ...
      CALL BRBTBP_PRC_15N (R,T,P,BR,BT,BP)
      CALL BSPCAR_15N (T,P,BR,BT,BP,BX1,BY1,BZ)
      BX= BY1
      BY=-BX1
      RETURN
      END
