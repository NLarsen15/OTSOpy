      subroutine igrf14 (isv,date,itype,alt,colat,elong,out)
c
...
c
     ...
     5          p(105),q(105),cl(13),sl(13),out(4)
...
c==========================================================================================
c
      SUBROUTINE IGRF14_GSW (DATE,XGSW,YGSW,ZGSW,HXGSW,HYGSW,HZGSW)
c
C  CALCULATES COMPONENTS OF THE MAIN (INTERNAL) GEOMAGNETIC FIELD IN THE GEOCENTRIC SOLAR-WIND
C  (GSW) COORDINATE SYSTEM, USING THE STANDALONE IGRF14 SYNTHESIS ABOVE (WITH ITS OWN
C  HARD-CODED IGRF COEFFICIENT TABLES) RATHER THAN THE GEOPACK2 G/H COEFFICIENT ARRAYS USED
C  BY IGRF_GSW_08 IN GEOPACK_DP.F. PROVIDED AS AN INDEPENDENT CROSS-CHECK OF THE GEOPACK-BASED
C  IGRF IMPLEMENTATION, WITH A MATCHING CARTESIAN-IN / CARTESIAN-OUT CALLING CONVENTION.
C
C  BEFORE THE FIRST CALL OF THIS SUBROUTINE, OR IF THE DATE/TIME OR SOLAR WIND VELOCITY
C  COMPONENTS HAVE CHANGED, THE GEO-GSW ROTATION MATRIX ELEMENTS MUST BE UPDATED BY CALLING
C  RECALC_08 (SAME REQUIREMENT AS IGRF_GSW_08 -- ONLY THE ROTATION MATRIX IS REUSED FROM
C  GEOPACK, THE FIELD SYNTHESIS ITSELF IS INDEPENDENT).
C
C-----INPUT PARAMETERS:
C
C     DATE  - DECIMAL YEAR (E.G. 2015.5). VALID 1900.0 TO 2030.0
C     XGSW,YGSW,ZGSW - CARTESIAN GEOCENTRIC SOLAR-WIND COORDINATES (IN UNITS RE=6371.2 KM)
C
C-----OUTPUT PARAMETERS:
C
C     HXGSW,HYGSW,HZGSW - CARTESIAN GSW COMPONENTS OF THE MAIN GEOMAGNETIC FIELD IN NANOTESLA
C
      IMPLICIT REAL*8 (A-H,O-Z)
      REAL*8 :: DATE, XGSW,YGSW,ZGSW, HXGSW,HYGSW,HZGSW
      REAL*8 :: XGEO,YGEO,ZGEO, R,THETA,PHI
      REAL*8 :: ALTKM, COLATDEG, ELONGDEG, RAD2DEG
      REAL*8 :: OUT(4), BR,BTHETA,BPHI, HXGEO,HYGEO,HZGEO
      INTEGER :: ISV, ITYPE
      ...
      ITYPE    = 2
      ALTKM    = R*6371.2D0
      COLATDEG = THETA*RAD2DEG
      ELONGDEG = PHI*RAD2DEG

      CALL IGRF14 (ISV,DATE,ITYPE,ALTKM,COLATDEG,ELONGDEG,OUT)

C     OUT(1)=X(NORTH), OUT(2)=Y(EAST), OUT(3)=Z(DOWN) -> LOCAL SPHERICAL (BR,BTHETA,BPHI),
C     USING THE STANDARD RELATION X=-BTHETA, Y=BPHI, Z=-BR:

      BR     = -OUT(3)
      BTHETA = -OUT(1)
      BPHI   =  OUT(2)

      CALL BSPCAR_08 (THETA,PHI,BR,BTHETA,BPHI,HXGEO,HYGEO,HZGEO)
...
c==========================================================================================
c
      SUBROUTINE IGRF14_GEO (DATE,XGEO,YGEO,ZGEO,HXGEO,HYGEO,HZGEO)
c
C  CALCULATES COMPONENTS OF THE MAIN (INTERNAL) GEOMAGNETIC FIELD DIRECTLY IN THE GEOCENTRIC
C  (GEO) COORDINATE SYSTEM, USING THE SAME STANDALONE IGRF14 SYNTHESIS AS IGRF14_GSW, BUT
C  WITHOUT THE GEO<->GSW ROUND-TRIP (NO CALL TO GEOGSW_08 AT EITHER END). CARTESIAN-IN /
C  CARTESIAN-OUT, BOTH IN GEO.
C
C-----INPUT PARAMETERS:
C
C     DATE  - DECIMAL YEAR (E.G. 2015.5). VALID 1900.0 TO 2030.0
C     XGEO,YGEO,ZGEO - CARTESIAN GEOCENTRIC (GEO) COORDINATES (IN UNITS RE=6371.2 KM)
C
C-----OUTPUT PARAMETERS:
C
C     HXGEO,HYGEO,HZGEO - CARTESIAN GEO COMPONENTS OF THE MAIN GEOMAGNETIC FIELD IN NANOTESLA
C
      IMPLICIT REAL*8 (A-H,O-Z)
      REAL*8 :: DATE, XGEO,YGEO,ZGEO, HXGEO,HYGEO,HZGEO
      REAL*8 :: R,THETA,PHI
      REAL*8 :: ALTKM, COLATDEG, ELONGDEG, RAD2DEG
      REAL*8 :: OUT(4), BR,BTHETA,BPHI
      INTEGER :: ISV, ITYPE
