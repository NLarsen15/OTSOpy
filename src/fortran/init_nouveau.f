...
C
       ...
       zDip = ct
C
       xD = cgsta*xDip - sgsta*yDip
       yD = sgsta*xDip + cgsta*yDip
       zD = zDip
C
       xSD = yD*zS - yS*zD
       ySD = zD*xS - zS*xD
...
C
      xGEO(1) =  cgsta*xGEI + sgsta*yGEI
      xGEO(2) = -sgsta*xGEI + cgsta*yGEI
        xGEO(3) =  zGEI
C
        RETURN
        END
...
!C       Modified by Paul OBrien: ierr set, but uses nearest date limit
...
!      IMPLICIT NONE
!C
!      REAL*8 F1,F2,DT,Ga(66),Ha(66),G65(66),H65(66),G70(66),H70(66),
...
!         Ha(N)=H2010(N)
!         IF (N.GT.45) GOTO 40
!         ga(N)=ga(N)+DG2010(N)*DT
!         ha(N)=ha(N)+DH2010(N)*DT
!40    CONTINUE
!      GOTO 300
!C
!C
...
!      F1=1.D0-F2
!      DO 55 N=1,66
!         ga(N)=G65(N)*F1+G70(N)*F2
!55       ha(N)=H65(N)*F1+H70(N)*F2
!      GOTO 300
!C
!C
...
!      F1=1.D0-F2
!      DO 65 N=1,66
!         ga(N)=G70(N)*F1+G75(N)*F2
!65       ha(N)=H70(N)*F1+H75(N)*F2
!      GOTO 300
!C
!C
...
!      F1=1.D0-F2
!      DO 75 N=1,66
!         ga(N)=G75(N)*F1+G80(N)*F2
!75       ha(N)=H75(N)*F1+H80(N)*F2
!      GOTO 300
!C
!C
...
!      F1=1.D0-F2
!      DO 85 N=1,66
!         ga(N)=G80(N)*F1+G85(N)*F2
!85       ha(N)=H80(N)*F1+H85(N)*F2
!      GOTO 300
!C
!C
...
!      F1=1.D0-F2
!      DO 95 N=1,66
!         ga(N)=G85(N)*F1+G90(N)*F2
!95       ha(N)=H85(N)*F1+H90(N)*F2
!      GOTO 300
!C
!C
...
!      F1=1.D0-F2
!      DO 105 N=1,66
!         ga(N)=G90(N)*F1+G95(N)*F2
!105      ha(N)=H90(N)*F1+H95(N)*F2
!      GOTO 300
!C
!C
...
!      F1=1.D0-F2
!      DO 115 N=1,66
!         ga(N)=G95(N)*F1+G00(N)*F2
!115      ha(N)=H95(N)*F1+H00(N)*F2
!      GOTO 300
!C
...
!      F1=1.D0-F2
!      DO 125 N=1,66
!         ga(N)=G00(N)*F1+G05(N)*F2
!125      ha(N)=H00(N)*F1+H05(N)*F2
!      GOTO 300
!C
...
!      F1=1.D0-F2
!      DO 135 N=1,66
!         ga(N)=G05(N)*F1+G2010(N)*F2
!135      ha(N)=H05(N)*F1+H2010(N)*F2
!      GOTO 300
!C
!C
!300      continue
...
C
c
      subroutine get_terms(ga,ha,alpha,beta,x0,y0,z0,b0)
c
      implicit none
c
      REAL*8 ga(66),ha(66),alpha,beta,x0,y0,z0
